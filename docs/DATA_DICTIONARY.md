...
| activity_type | VARCHAR(20) | BILLABLE / INTERNAL / SICK / VACATION / PARENTAL_LEAVE / WELFARE_LEAVE / FLEX_LEAVE / CHILD_CARE_LEAVE |
...
Business meaning: employee time tracking, not just consultant timesheets. **Generates no accounting postings at all** — purely operational data (no effect on the P&L, independent of `salary_generator`). **Salary treatment differs by absence type**: SICK, PARENTAL_LEAVE, WELFARE_LEAVE, FLEX_LEAVE, and CHILD_CARE_LEAVE are all paid at full ordinary salary (the employer pays and reclaims sykepenger/foreldrepenger/omsorgspenger from NAV) — matching "any other day". **VACATION is the one exception**: it is *not* paid via ordinary monthly salary — it's covered by feriepenger (holiday pay), which this project already models independently as the June salary swap (`seed/payroll.py::calc_june_salary` — the entire normal June salary is replaced by accrued feriepenger). That mechanism has no dependency on which specific calendar days are marked `VACATION` in `hour_entries` — the two systems just happen to both be "about vacation" without being wired together, which is fine since `salary_generator` never reads `hour_entries` at all. Limitation: BILLABLE only for projects whose customer is already onboarded and hasn't churned yet — otherwise the whole day is logged as INTERNAL.
...
**Absence types, all four departments (`generators/leave_events.py` + `generators/vacation.py` + `generators/flex_leave.py`)**: checked in this order for every employee (except E05), each day — the first match wins and no department-specific hours are logged that day:
1. **SICK (long)** / **PARENTAL_LEAVE** / **WELFARE_LEAVE** — long-absence periods, `leave_events.employee_leave_periods()`.
2. **VACATION** — a hard 25 working days/year.
3. **FLEX_LEAVE** — 12 days/year, `flex_leave.employee_flex_leave_days()`.
4. **CHILD_CARE_LEAVE** — 10 days/year, `flex_leave.employee_child_care_leave_days()`.
5. A short, self-certified SICK day — ~5% per-day chance, independent of the above.
...
- **PARENTAL_LEAVE** — Norwegian-style `foreldrepermisjon`, up to **52 weeks total per event**, split into a **primary block** (20-39 weeks) at one employee, plus a separate **"fedrekvote" (father's quota) block** (~12-14 weeks, ~3 months) at a **different, randomly chosen employee**, starting the day after the primary block ends. Rolled independently **every year** an employee is active (6%/year for the primary trigger, ~1 case/year across a 17-person team), with a 2-year cooldown after a triggered *primary* event (the received father's-quota block is exempt from that employee's own cooldown — it isn't their event). Deliberately **not gender-assigned** — `EmployeeSeed` has no gender field, so both roles are picked symmetrically; the "different employee" stands in for a second parent without asserting any family relationship between two of these 17 fictional people (`leave_events._father_quota_assignments()`, a company-wide pass, not a per-employee one, since it writes into someone else's periods).
- **WELFARE_LEAVE** — short (1-3 days), occasional (15%/year), no cooldown (unlike parental leave, it can recur).
- **FLEX_LEAVE** ("dager til avspasering") — a hard 12 days/year, taken in blocks of at most 3 consecutive working days, with a mandatory 16-day gap before the next block (`flex_leave.employee_flex_leave_days`).
- **CHILD_CARE_LEAVE** ("omsorgsdager", sick-child care) — a hard 10 days/year, blocks of at most 2 days, no mandated gap (a sick child doesn't wait 16 days for the next one) — though blocks are still kept at least 1 day apart so they can never silently merge into a longer run than the 2-day cap implies.

All of these are precomputed deterministically per `employee_id` (`employee_leave_periods()` / `employee_vacation_days()` / `employee_flex_leave_days()` / `employee_child_care_leave_days()`, memoized like `client_events.py`/`company_events.py` — pure functions, no "today" input, safe across the backfill's separate OS processes) and, while active, replace that whole day with a single 0h entry of the matching type — no BILLABLE/INTERNAL/Salg/Okonomi hours logged for that employee that day.
